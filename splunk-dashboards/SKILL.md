...
| `enableSmartSources` | boolean | No | Accelerate with data model if available |
...
| `splunk.linkList` | List of clickable links | `items`, `orientation` |
...
| `input.radio` | Radio button group | `token`, `items`, `defaultValue` |
...
> **IMPORTANT:** Dynamic dropdowns require the full `statics` + `formattedStatics` + `formatByType` + `objects()` pattern. The simpler `prepend(prependItems)` syntax does NOT work -- items will not render. The search must produce `label` and `value` columns (use `| rename <field> AS label | eval value=label`).
...
**How the `statics` array works:** The `statics` array is a columnar format: `[["label1", "label2", ...], ["value1", "value2", ...]]`. The first sub-array contains labels, the second contains values. For a single "All" option: `[["All"], ["*"]]`.

### 4.5 Multiselect Input

> **IMPORTANT:** Multiselect uses the same `formattedStatics` + `objects()` pattern as dropdown.
...
The `applicationProperties` section sets global dashboard properties like theme:
...
`"theme"` values: `"dark"` or `"light"`. The theme here should match the `theme` attribute on the XML `<dashboard>` wrapper element.

---

## 8. Dynamic Options Syntax (DOS)
...
### 8.2 Selector Functions (pick data)
...
### 8.3 Formatting Functions (transform data)
...
### 8.4 Context — Configuration Store
...
## 9. Interactions (Drilldown & Tokens)

### 9.1 Setting Tokens on Click (drilldown.setToken)
...
## 10. Standard Patterns & Templates

### 10.1 Minimal Dashboard Skeleton
...
### 10.2 Security SOC Dashboard Pattern
...
### 10.3 Color Conventions (Dark Theme)
...
## 12. Common SPL Patterns for Dashboard Studio
