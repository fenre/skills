...
DEFAULT_RENDER_DIR_NAME="splunk-observability-azure-integration-rendered"
...
Direct-secret flags below are REJECTED:
  --secret --client-secret --token --password --app-secret
...
Refusing direct-secret flag --${name}. Use file-based equivalents:
  --app-id-file PATH    Azure AD application (client) ID
  --secret-file PATH    Azure AD client secret
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --app-id-file) APP_ID_FILE="$2"; shift ;;
        --secret-file) SECRET_FILE="$2"; shift ;;
...
run_renderer() {
    ...
    [[ -n "${APP_ID_FILE}" ]] && args+=("--app-id-file" "${APP_ID_FILE}")
    [[ -n "${SECRET_FILE}" ]] && args+=("--secret-file" "${SECRET_FILE}")
...
run_validate() {
    ...
    [[ "${SAZURE_VALIDATE_LIVE:-}" == "true" ]] && args+=(--live)
...
case "${MODE}" in
    ...
    quickstart)
        ...
        echo "      --app-id-file /tmp/azure-app-id.txt \\"
        echo "      --secret-file /tmp/azure-secret.txt"
        ;;
