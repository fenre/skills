...
CREDENTIAL_FIELDS: tuple[str, ...] = ("projectKey",)
...
def upsert(
    ...
    *,
    ...
    After a successful apply, stores SHA-256 hashes of the key files
    ...
    # Resolve actual projectKey values from files.
    if key_files:
        new_psk = []
        for entry in payload.get("projectServiceKeys", []):
            if isinstance(entry, dict):
                # The key_file is not stored in the payload — the caller passes it separately.
                # Use the first key_file provided for all projects (single SA key workflow).
                project_key = ""
                if key_files:
                    try:
                        project_key = read_secret_file(key_files[0])
    ...
    if match:
        ...
        _record_credential_hashes(state_dir, key_files or [])
...
def _record_credential_hashes(state_dir: Path, key_files: list[str]) -> None:
    hashes: dict[str, str] = {}
    for path in key_files:
        if path:
    ...
    if hashes:
        ...
        _save_cred_hashes(state_dir, {"project_key_sha256": existing_keys})
...
def _reject_direct_secret_flags() -> None:
    for arg in sys.argv[1:]:
        ...
        if flag in _REJECTED_SECRET_FLAGS:
            print(
                f"FAIL: refusing direct-secret flag {flag}. Use --token-file or --key-file "
                f"(all chmod 600).",
                flush=True,
            )
            sys.exit(2)
...
def main() -> int:
    ...
    try:
        if args.command == "list":
            ...
            if not args.key_files:
                raise ApiError("--key-file is required for `check-drift`")
            warnings = check_credential_drift(state_dir, args.key_files)
            if warnings:
                for w in warnings:
