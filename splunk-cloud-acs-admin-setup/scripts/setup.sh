...
DEFAULT_RENDER_DIR_NAME="splunk-cloud-acs-admin-rendered"
...
Options:
  ...
  --json
  --output-dir PATH
  --modules CSV (subset of: allowlists,indexes,hec-tokens,users,roles,capabilities,app-permissions,private-connectivity,outbound-ports,ddss,limits,maintenance-windows,restarts,apps,tokens,deployment,license,observability)
  --admin-plan-file PATH (JSON)
...
Examples:
  ...
  $(basename "$0") --phase render --admin-plan-file acs-admin-plan.json
  $(basename "$0") --phase audit
  ACCEPT_ACS_ADMIN_MUTATION=true $(basename "$0") --phase apply --admin-plan-file acs-admin-plan.json
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --modules) require_arg "$1" $# || exit 1; MODULES="$2"; shift 2 ;;
        --admin-plan-file) require_arg "$1" $# || exit 1; ADMIN_PLAN_FILE="$2"; shift 2 ;;
...
build_renderer_args() {
    RENDER_ARGS=(
        ...
        --modules "${MODULES}"
        --admin-plan-file "${ADMIN_PLAN_FILE}"
        --features "${FEATURES}"
    ...
    case "${PHASE}" in
        render)
            ...
            if [[ "${APPLY}" == "true" ]]; then
                ...
                run_rendered_script apply-admin-plan.sh
                run_rendered_script wait-for-ready.sh
            fi
            ;;
        preflight) render_assets; run_rendered_script preflight.sh ;;
        apply) render_assets; run_rendered_script preflight.sh; run_rendered_script apply-ipv4.sh; run_rendered_script apply-ipv6.sh; run_rendered_script apply-admin-plan.sh; run_rendered_script wait-for-ready.sh ;;
        status) run_rendered_script wait-for-ready.sh ;;
        audit) render_assets; run_rendered_script audit.sh ;;
        validate) render_assets; run_rendered_script inventory.sh; run_rendered_script audit.sh ;;
        all) render_assets; run_rendered_script preflight.sh; run_rendered_script inventory.sh; run_rendered_script apply-ipv4.sh; run_rendered_script apply-ipv6.sh; run_rendered_script apply-admin-plan.sh; run_rendered_script wait-for-ready.sh; run_rendered_script audit.sh ;;
    esac
}
