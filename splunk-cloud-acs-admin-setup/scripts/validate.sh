...
usage() {
    local exit_code="${1:-0}"
    cat <<EOF
Splunk Cloud ACS Admin Validation
...
required=(README.md admin-commands.sh apply-admin-plan.sh metadata.json plan.json preflight.sh apply-ipv4.sh apply-ipv6.sh inventory.sh private-connectivity-rest.sh wait-for-ready.sh audit.sh)
missing=()
