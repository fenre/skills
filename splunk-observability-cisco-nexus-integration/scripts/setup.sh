...
Modes:
  ...
  --apply             Merge overlay onto the existing Splunk OTel collector helm
                      ...
                      --accept-k8s-apply, an existing 'cisco-nexus-ssh' Secret
                      in the collector namespace, and O11Y_TOKEN_FILE env.
...
if [[ "${MODE_APPLY}" == "true" ]]; then
    APPLY_SCRIPT="${OUTPUT_DIR}/scripts/apply-nexus-overlay.sh"
    ...
    show_kube_context
    log "Applying Cisco Nexus overlay via rendered helper..."
