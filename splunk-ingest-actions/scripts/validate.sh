...
required=(README.md metadata.json outputs.conf ruleset.json props_transforms_preview.conf apply.sh status.sh)
missing=()
...
if [[ -f "${render_dir}/ruleset.json" ]]; then
    if ! python3 -c "import json,sys; json.load(open(sys.argv[1]))" "${render_dir}/ruleset.json" 2>/dev/null; then
        missing+=("ruleset.json invalid JSON")
        ok=false
    fi
fi
...
if [[ "${JSON_OUTPUT}" == "true" ]]; then
    ...
    if [[ "${ok}" == "true" ]]; then
        ...
        log "ERROR: Missing or invalid Ingest Actions assets under ${render_dir}: ${missing[*]}"
    fi
fi
