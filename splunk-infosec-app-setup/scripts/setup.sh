...
SECURITY_INDEXES="security,endpoint,network"
CLOUD_IDM_REQUIRED="false"
LOOKUP_OWNER_APP="infosec_app_for_splunk"
APP_ID="4240"
PRODUCT_NAME="InfoSec App Setup"
...
Options:
  --render                         Render reviewable InfoSec setup assets
  --install                        Install InfoSec app through splunk-app-install
  ...
  --security-indexes LIST          Security indexes for dashboards
  --cloud-idm-required true|false  Add Cloud IDM support-request note
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --security-indexes) require_arg "$1" $# || exit 1; SECURITY_INDEXES="$2"; shift 2 ;;
        --cloud-idm-required) require_arg "$1" $# || exit 1; CLOUD_IDM_REQUIRED="$2"; shift 2 ;;
...
case "${CLOUD_IDM_REQUIRED}" in true|false) ;; *) echo "ERROR: --cloud-idm-required must be true or false." >&2; exit 1 ;; esac
...
RENDER_CMD=(python3 "${RENDER_SCRIPT}" --phase render --platform "${PLATFORM}" --es-app "${ES_APP}" --security-indexes "${SECURITY_INDEXES}" --cloud-idm-required "${CLOUD_IDM_REQUIRED}" --lookup-owner-app "${LOOKUP_OWNER_APP}")
...
if [[ "${DRY_RUN}" == "true" ]]; then
    if [[ "${JSON}" == "true" ]]; then
        rf_emit_action_plan_json "${SKILL_NAME}" "${PRODUCT_NAME}" "${PHASES_JOIN}" "$(rf_join_unit "${RENDER_CMD[@]}")" "$(rf_join_unit "${RF_INSTALL_CMD[@]}")" "$(rf_join_unit "${VALIDATE_CMD[@]}")" "$(rf_join_unit "Installs InfoSec app only; data-source, CIM, and lookup governance remain delegated.")"
    else
