...
LOOKUP_OWNER_APP="search"
LOOKUP_SCOPE="both"
SHC_MODE="false"
APP_ID="1724"
PRODUCT_NAME="Splunk Lookup File Editing Setup"
...
Options:
  --render                         Render reviewable Lookup Editor assets
  --install                        Install Lookup Editor through splunk-app-install
  ...
  --lookup-owner-app APP           App namespace to inventory
  --lookup-scope csv|kvstore|both  Lookup inventory scope
  --shc-mode true|false            Render SHC allowRestReplay notes
  --help                           Show this help
EOF
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --lookup-scope) require_arg "$1" $# || exit 1; LOOKUP_SCOPE="$2"; shift 2 ;;
        --shc-mode) require_arg "$1" $# || exit 1; SHC_MODE="$2"; shift 2 ;;
        --help|-h) usage; exit 0 ;;
...
case "${LOOKUP_SCOPE}" in csv|kvstore|both) ;; *) echo "ERROR: invalid --lookup-scope." >&2; exit 1 ;; esac
case "${SHC_MODE}" in true|false) ;; *) echo "ERROR: --shc-mode must be true or false." >&2; exit 1 ;; esac
...
if [[ "${DRY_RUN}" == "true" ]]; then
    if [[ "${JSON}" == "true" ]]; then
        rf_emit_action_plan_json "${SKILL_NAME}" "${PRODUCT_NAME}" "${PHASES_JOIN}" "$(rf_join_unit "${RENDER_CMD[@]}")" "$(rf_join_unit "${RF_INSTALL_CMD[@]}")" "$(rf_join_unit "${VALIDATE_CMD[@]}")" "$(rf_join_unit "Installs Lookup Editor only; lookup content, ACL, and KV Store changes remain delegated.")"
    else
